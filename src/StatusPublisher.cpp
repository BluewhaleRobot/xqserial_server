...
void StatusPublisher::Update_sonar(const char data[], unsigned int len)
{
    int i=0,j=0;
    int * receive_byte;
    static unsigned char last_str[2]={0x00,0x00};
    ...
    for(i=0;i<len;i++)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                {
                    //std::cout<<"runup4 "<<std::endl;
                    boost::mutex::scoped_lock lock(mMutex_sonar);
                    //当前包已经处理完成，开始处理
...
void StatusPublisher::Refresh()
{
    ...
    if(mbUpdated)
    {
      ...
      //超声波测距
      boost::mutex::scoped_lock lock2(mMutex_sonar);
      ...
      if(distance_sum_i%10==0)
      {
        ...
        distances_[0] = sonar_status.sonar_distance[0];
        distances_[1] = sonar_status.sonar_distance[1];
        distances_[2] = sonar_status.sonar_distance[2];
        distances_[3] = sonar_status.sonar_distance[3];
        ...
        if(distances_[0]>0.1)
        {
          ...
          if(distances_[0]<tran_dist_ && move_forward_flag)
          {
        ...
        if(distances_[1]>0.1)
        {
          ...
          if(distances_[1]<tran_dist_ && move_forward_flag)
          {
            move_forward_flag = false;
        ...
        if(distances_[2]>0.1)
        {
          ...
          if(distances_[2]<tran_dist_ && move_forward_flag)
          {
            move_forward_flag = false;
        ...
        if(distances_[3]>0.1)
        {
          ...
          if(distances_[3]<tran_dist_ && move_forward_flag)
          {
            move_forward_flag = false;
            //ROS_ERROR("sonar4 %f",distances_[3]);
          }
        }
          //ROS_ERROR("move_forward_flag %d %f %f",(int)move_forward_flag,distances_[1],car_status.sonar_distance[1]);
      }
      else
      {
        //匀减速模型
      ...
      if(clearArea_nums>0)
      {
        ...
        if(hbz2_num==0)
        {
          ...
          {
            *cleariter_x=0.1+0.35;
            *cleariter_y=k*0.04+0.15;
            *cleariter_z=0.0;
          }
          for(int k=0;k<5;k++,++cleariter_x, ++cleariter_y,++cleariter_z)
          {
            *cleariter_x=0.05+0.35;
            *cleariter_y=k*0.04+0.15;
            *cleariter_z=0.0;
          }
        }
...
{
  boost::mutex::scoped_lock lock2(mMutex_sonar);
  distances[0]=std::min(distances_[0],distances_[2]);
  distances[1]=std::min(distances_[1],distances_[3]);
}
